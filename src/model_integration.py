...
print("Done!")
'''
def create_flask_app(stage1_path: str, stage2_path: str):
    ...
    app     = Flask(__name__)
    pipeline = DFUPipeline(stage1_path, stage2_path)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DFU Pipeline")
    parser.add_argument("--stage1",  default="dfu_model.h5",       help="Path to Stage 1 .h5 model")
    parser.add_argument("--stage2",  default="dfu_wagner_v4.pth",   help="Path to Stage 2 .pth model")
    parser.add_argument("--image",   default=None,                  help="Image to predict")
    parser.add_argument("--batch",   default=None,                  help="Folder for batch prediction")
    parser.add_argument("--eval",    default=None,                  help="Test folder for evaluation")
    ...
    if args.serve:
        app = create_flask_app(args.stage1, args.stage2)
        print(f"[Server] Starting on http://0.0.0.0:{args.port}")
        app.run(host="0.0.0.0", port=args.port, debug=False)

    else:
        pipe = DFUPipeline(args.stage1, args.stage2)

        if args.image:
            result = pipe.predict(args.image)
            pipe._pretty_print(args.image, result)

        elif args.batch:
            pipe.batch_predict(args.batch)

        elif args.eval:
            pipe.evaluate_pipeline(args.eval)

        else:
            print("Provide --image, --batch, --eval, or --serve. Run with -h for help.")
'''